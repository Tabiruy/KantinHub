...
async function fetchKantin() {
  ...
  container.innerHTML =
    '<div class="text-center w-100 my-4"><div class="spinner-border text-warning" role="status"></div><p class="mt-2 text-muted">Memuat kantin...</p></div>';
...
async function fetchMenu(kantinId) {
  ...
  container.innerHTML =
    '<div class="text-center w-100 my-4"><div class="spinner-border text-warning" role="status"></div><p class="mt-2 text-muted">Memuat menu...</p></div>';
...
async function addReview() {
  const nameInput =
    document.getElementById("reviewerName").value || activeUser || "Anonim";
  ...
  const { data, error } = await _supabase
    .from("reviews")
    .insert([{ username: nameInput, review_text: textInput }]);
...
function renderKantin() {
  ...
  if (dataKantin.length === 0) {
    container.innerHTML =
      '<p class="text-center w-100 text-muted">Belum ada kantin terdaftar.</p>';
    return;
  }
...
function addToCart(menuId) {
  ...
  if (item) {
    if (cart.length > 0 && cart[0].kantin_id !== item.kantin_id) {
      if (
        confirm(
          "Keranjang Anda berisi makanan dari kantin lain. Ingin menghapus keranjang dan memesan dari kantin ini?",
        )
      ) {
        cart = [];
      } else {
...
function showSection(sectionId) {
  ...
  document.body.classList.toggle("is-checkout", sectionId === "checkout");
...
function renderCheckout() {
  ...
  if (cart.length === 0) {
    ...
    document.querySelector("#checkout-section button.btn-warning").disabled =
      true;
    return;
  }
...
function increaseQty(id) {
  const item =
    cart.find((m) => m.id === id) || dataMenu.find((m) => m.id === id);
  if (item) {
    cart.push({ ...item });
    updateCartUI();
    renderCheckout();
...
async function fetchHistory() {
  ...
  if (!activeUser) {
    container.innerHTML =
      "<p class='text-center text-muted mt-4'>Silakan login terlebih dahulu untuk melihat riwayat.</p>";
    return;
  }

  container.innerHTML =
    '<div class="text-center w-100 my-4"><div class="spinner-border text-warning" role="status"></div><p class="mt-2 text-muted">Memuat riwayat...</p></div>';
...
async function fetchAdminMenus() {
  ...
  const { data: menuData, error: menuErr } = await _supabase
    .from("menu")
    .select("*, kantin(nama)")
    .order("kantin_id", { ascending: true });
...
async function updateMenuPhoto(id) {
  const newUrl = prompt(
    "Masukkan URL foto baru (disarankan dari Unsplash/Pexels):",
  );
  if (newUrl) {
